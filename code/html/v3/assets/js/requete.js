...
function launchPy(type, action, step) {
    ...
    httpRequest3.send("type="+type+"&action="+action+"&step="+step);
...
function calibrate(type, action, step) {
    httpRequest4 = new XMLHttpRequest();
    if (!httpRequest4) {
        alert('erreur Requette');
    }
    httpRequest4.responseType = "json";
    httpRequest4.open('POST', 'assets/php/launchPy.php');
    httpRequest4.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    httpRequest4.send("type="+type+"&action="+action+"&step="+step);
    httpRequest4.onload = function() {
        if(httpRequest4.readyState == 4 && httpRequest4.status == 200) {
            sortie = httpRequest4.response;
            endCalibration();
            console.log(sortie);
        }
